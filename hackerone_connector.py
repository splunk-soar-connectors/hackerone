...
class HackerOneConnector(BaseConnector):
    ...
    def _validate_integer(self, action_result, parameter, key):
        if parameter is not None:
            try:
                if not float(parameter).is_integer():
                    return action_result.set_status(
                        phantom.APP_ERROR, INT_VALIDATION_ERR_MSG.format(key)
                    ), None
            ...
            if parameter < 0:
                return action_result.set_status(
                    phantom.APP_ERROR, NEG_INT_VALIDATION_ERR_MSG.format(key)
                ), None
    ...
    def _get_phantom_data(self, endpoint):
        ...
        try:
            ...
            try:
                ...
                self.__print(
                    "Error parsing JSON Object: {}".format(
            ...
            if code == 200:
                self.__print(
                    "Finish: _get_phantom_data(): {0}".format(datetime.datetime.now())
                )
                return content
            else:
                self.__print(code)
    ...
    def _post_phantom_data(self, url, dictionary):
        ...
        try:
            ...
            if code >= 200 and code < 300:
                self.__print(
                    "Finish: _post_phantom_data(): {0}".format(datetime.datetime.now())
                )
                return code
            else:
                self.__print(code)
    ...
    def _delete_phantom_data(self, url):
        self.__print(
            "Start: _delete_phantom_data(): {0}".format(datetime.datetime.now())
        )
        try:
            self.__print(url)
            ...
            if code >= 200 and code < 300:
                self.__print(
                    "Finish: _delete_phantom_data(): {0}".format(
    ...
    def _put_rest_data(self, url, dictionary):
        ...
        try:
            ...
            if code >= 200 and code < 300:
                self.__print(
                    "Finish: _put_rest_data(): {0}".format(datetime.datetime.now())
                )
                return code
            else:
                self.__print(code)
    ...
    def _post_rest_data(self, url, dictionary):
        ...
        try:
            ...
            if code >= 200 and code < 300:
                self.__print(
                    "Finish: _post_rest_data(): {0}".format(datetime.datetime.now())
                )
                return code
            else:
                self.__print(code)
        ...
        try:
            data = {
                "data": {
                    ...
                    "attributes": {"reference": tracking_id},
                }
            }
            ...
            if self._post_rest_data(url, data):
                ...
                return action_result.set_status(
                    phantom.APP_SUCCESS, "Successfully updated tracking id"
                )
            else:
                self.__print("Failed to update tracking id.")
                return action_result.set_status(
                    phantom.APP_ERROR, "Failed to update tracking id"
                )
        except Exception as e:
            err = self._get_error_message_from_exception(e)
    ...
    def _unassign_report(self, param, action_result):
        ...
        try:
            ...
            if self._put_rest_data(url, data):
                ...
                return action_result.set_status(
                    phantom.APP_SUCCESS, "Successfully removed report assignment"
                )
            else:
                self.__print("Failed to remove report assignment")
                return action_result.set_status(
                    phantom.APP_ERROR, "Failed to remove report assignment"
                )
        except Exception as e:
            err = self._get_error_message_from_exception(e)
            self.__print("Exception occurred while updating tracking id")
    ...
    def _get_cvf(self, core_report):
        try:
            core_report["severity_cvf"] = (
                "CVSS:3.0/AV:{0}/AC:{1}/PR:{2}/UI:{3}/S:{4}/C:{5}/I:{6}/A:{7}".format(
    ...
    def _parse_report(self, report):
        ...
        with open(
            os.path.join(__location__, "files", "hackerone_report_template.json")
        ) as json_file:
    ...
    def _get_filtered_reports(self, program, state, assignment, add_comments, date):
        ...
        try:
            ...
            reports, links = self._get_rest_data(
                "https://api.hackerone.com/v1/reports", url_params
            )
    ...
    def _get_reports(self, param, action_result):
        try:
            ...
            reports = self._get_filtered_reports(
                program, state, assignment, add_comments, None
            )
            if not reports:
                self.__print("No reports found")
                return action_result.set_status(
                    phantom.APP_ERROR, "Failed to get reports"
                )
            ...
            return action_result.set_status(
                phantom.APP_SUCCESS, "Successfully collected reports"
            )
        except Exception as e:
            err = self._get_error_message_from_exception(e)
    ...
    def _get_updated_reports(self, param, action_result):
        try:
            ...
            reports = self._get_filtered_reports(
                program, state, assignment, add_comments, date
            )
            if not reports:
                self.__print("No reports found")
                return action_result.set_status(
    ...
    def _extract_urls(self, source):
        ...
        for value in re.finditer(url_pattern, source):
            ...
            if domain and len(re.findall(invalid_pattern, domain)) == 0:
                ...
                artifact["source_data_identifier"] = "Hacker1 url - {0}".format(
                    url[:60]
                )
                artifact["severity"] = "medium"
                cef = {}
    ...
    def _test(self, action_result, param):
        ...
        try:
            config = self.get_config()
            url_params = {
                "filter[program][]": self._handle_unicode_for_input_str(
                    config["program_name"]
                ),
                "page[size]": 1,
            }
            reports, _ = self._get_rest_data(
                "https://api.hackerone.com/v1/reports", url_params
            )
            if reports:
                return action_result.set_status(
    ...
    def _on_poll(self, param):
        ...
        try:
            ...
            current_state["time_marker"] = current_time_marker.strftime(
                "%Y-%m-%dT%H:%M:%S.%fZ"
            )
        ...
        assignment = config.get("assignment_filter")
        ...
        if reports is not None:
            ...
            for report in reports:
                ...
                container_name = "H1 {0}: {1}".format(
                    report["id"], re.sub(r"[^\x00-\x7f]", r"", report["title"])
                )
                endpoint = (
                    '{0}/rest/container?_filter_name__startswith="H1 {1}"'.format(
                        login_url, report["id"]
                    )
                )
                ...
                container = {}
                container["source_data_identifier"] = "HackerOne Report - {0}".format(
                    report["id"]
                )
                ...
                try:
                    for comment in report["comments"]:
                        ...
                        artifact["source_data_identifier"] = (
                            "HackerOne report - {0}: Comment - {1}".format(
                ...
                try:
                    for attachment in report["attachments"]:
                        ...
                        artifact["source_data_identifier"] = (
                            "HackerOne report - {0}: Attachment - {1}".format(
                    ...
                    pass
                try:
                    url_artifacts = self._extract_urls(
                        report["vulnerability_information"]
                    )
                ...
                if not existing_container:
                    ...
                    endpoint = "{0}/rest/container/{1}/artifacts?page_size=0".format(
                        login_url, existing_container
                    )
                    ...
                    for container_artifact in container_artifacts:
                        if "report" == container_artifact["label"]:
                            endpoint = "{0}rest/artifact/{1}".format(
                                login_url, container_artifact["id"]
                            )
                            self._delete_phantom_data(endpoint)
                            updated_report = True
                        else:
                            duplicates[container_artifact["name"]] = container_artifact[
                                "id"
                            ]
                    added_report = False
                    for artifact in artifacts:
                        if "report" == artifact["label"]:
                            if not added_report:
                                ...
                                artifact["source_data_identifier"] = (
                                    "{0}-HackerOne-Report".format(report["id"])
                                )
            ...
            return self.set_status(
                phantom.APP_SUCCESS, "Successfully stored report data"
            )
        else:
